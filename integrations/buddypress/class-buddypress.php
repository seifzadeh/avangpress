...
/**
 * Class Avangpress_BuddyPress_Integration
 *
 * @ignore
 */
class Avangpress_BuddyPress_Integration extends Avangpress_User_Integration {
	...
	public function add_hooks() {
		...
		if (is_multisite()) {
			 ...
			 * activation process.
			 *
			 * To avoid all signups being subscribed to the AvangPress list until they
		 ...
		 * plugins to delay the AvangPress signup process.
		 *
		 * Plugins can hook into the 'avangpress_integration_buddypress_should_subscribe' filter to prevent
		 * subscriptions from taking place:
		 *
		 * add_filter( 'avangpress_integration_buddypress_should_subscribe', '__return_false' );
		 *
		 * The plugin would then then call:
		 *
		 * do_action( 'avangpress_integration_buddypress_subscribe_user', $user_id );
		 *
		 * to perform the subscription at a later point.
		 */
		add_action('avangpress_integration_buddypress_subscribe_user', array($this, 'subscribe_buddypress_user'), 10, 1);
	...
	public function subscribe_from_form($user_id, $user_login, $user_password, $user_email) {
		...
		/**
		 * Allow other plugins to prevent the AvangPress sign-up.
		 *
		 * @param bool $subscribe False does not subscribe the user.
		 * @param int $user_id The user ID to subscribe
		 */
		$subscribe = apply_filters('avangpress_integration_buddypress_should_subscribe', $subscribe, $user_id);
	...
	public function store_usermeta($usermeta) {
		...
		if ($this->triggered()) {
			$usermeta['avangpress_subscribe'] = '1';
		}
	...
	public function subscribe_from_usermeta($user_id, $key, $userdata) {
		...
		if (empty($meta['avangpress_subscribe'])) {
			return false;
		}
		...
		/**
		 * @ignore Documented elsewhere, see Avangpress_BuddyPress_Integration::subscribe_from_form.
		 */
		$subscribe = apply_filters('avangpress_integration_buddypress_should_subscribe', $subscribe, $user_id);
		if (!$subscribe) {
			return false;
	...
	/**
	 * Subscribes a user to AvangPress list(s).
	 *
	 * @param int $user_id The user ID to subscribe
	 * @return bool
	 */
