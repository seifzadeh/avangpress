...
/**
 * Class Avangpress_Ninja_Forms_v2_Integration
 *
 * @ignore
 */
class Avangpress_Ninja_Forms_v2_Integration extends Avangpress_Integration {
	...
	public function register_field() {
		$args = array(
			'name' => __('AvangPress', 'ninja-forms'),
			'edit_function' => '',
		...
		ninja_forms_register_field('avangpress-subscribe', $args);
	}
	...
	public function process($id, $value) {
		...
		foreach ($fields as $field_id => $field_value) {
			...
			// try admin label for "avangpress-" prefixed fields, otherwise use general label
			$label = $ninja_forms_processing->get_field_setting($field_id, 'admin_label');
			if (empty($label) || stripos($label, 'avangpress-') !== 0) {
		...
		// guess mail variables
		$parser = new Avangpress_Field_Guesser($pretty);
