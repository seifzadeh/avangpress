...
/**
 * Class Avangpress_Contact_Form_7_Integration
 *
 * @ignore
 */
class Avangpress_Contact_Form_7_Integration extends Avangpress_Integration {
	...
	public function init() {
		...
		if (function_exists('wpcf7_add_form_tag')) {
			wpcf7_add_form_tag('avangpress_checkbox', array($this, 'shortcode'));
		} else {
			wpcf7_add_shortcode('avangpress_checkbox', array($this, 'shortcode'));
		}
	...
	public function checkbox_was_checked() {
		...
		return (isset($data[$this->checkbox_name]) && $data[$this->checkbox_name] == 1)
			|| (isset($data['avangpress-subscribe']) && $data['avangpress-subscribe'] == 1);
	}

	/**
	 * Alter Contact Form 7 data.
	 *
	 * Adds avangpress_checkbox to post data so users can use `avangpress_checkbox` in their email templates
	 *
	 * @param array $data
	 * @return array
	 */
	public function alter_cf7_data($data = array()) {
		$data['avangpress_checkbox'] = $this->checkbox_was_checked() ? __('Yes') : __('No');
		return $data;
	}
	 ...
	 *
	 * @todo improve smart guessing based on selected AvangPress lists
	 *
	...
	public function process($cf7_form) {
		...
		$parser = new Avangpress_Field_Guesser($this->get_data());
