...
class Avangpress_WPForms_Field extends WPForms_Field {
	...
	public function init() {
		...
		$this->name = 'AvangPress';
		$this->type = 'mail';
		$this->icon = 'fa-envelope-o';
		$this->order = 21;
		$this->defaults = array(
			0 => array(
				'label' => __('Sign-up to our newsletter?', 'avangpress'),
				'value' => '1',
				'default' => '',
			),
		);
	...
	public function field_options($field) {
		...
		// AvangPress list
		$this->field_option_mail_list($field);
	...
	private function field_option_mail_list($field) {
		...
		$option_label = $this->field_element(
			...
			array(
				'slug' => 'mail-list',
				'value' => __('AvangPress list', 'avangpress'),
				'tooltip' => $tooltip,
			),
			false
		);

		$option_select = sprintf('<select name="fields[%s][mail_list]" data-field-id="%d" data-field-type="%s">', $field['id'], $field['id'], $this->type);
		$lists = $mail->get_cached_lists();
		foreach ($lists as $list) {
			$option_select .= sprintf('<option value="%s" %s>%s</option>', $list->id, selected($list->id, $field['mail_list'], false), $list->name);
		}
	...
	private function field_option_choices($field) {
		$tooltip = __('Set your sign-up label text and whether it should be pre-checked.', 'avangpress');
		...
		$option_label = $this->field_element(
			...
			array(
				'slug' => 'mail-checkbox',
				'value' => __('Sign-up checkbox', 'avangpress'),
				'tooltip' => $tooltip,
			),
			false
		);
