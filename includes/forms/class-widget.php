...
/**
 * Adds Avangpress_Widget widget.
 *
 * @ignore
 */
class Avangpress_Form_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$this->default_instance_settings['title'] = __( 'Newsletter', 'avangpress' );

		parent::__construct(
			'avangpress_form_widget', // Base ID
			__( 'Mail Sign-Up Form', 'avangpress' ), // Name
			array(
				'description' => __( 'Displays your Mail for WordPress sign-up form', 'avangpress' ),
			)
		);
	}
	...
	public function widget( $args, $instance_settings ) {
		...
		avangpress_show_form( $instance_settings['form_id'] );
	...
	public function form( $settings ) {
		...
		?>
        <p>
			<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Title:', 'avangpress' ); ?></label>
		 ...
		 * @param Avangpress_Form_Widget $this
		 * @ignore
		 */
		do_action( 'avangpress_form_widget_form', $settings, $this );
		?>

        <p class="help">
			<?php printf( __( 'You can edit your sign-up form in the <a href="%s">Mail for WordPress form settings</a>.', 'avangpress' ), admin_url( 'admin.php?page=avangpress-forms' ) ); ?>
        </p>
		<?php
	}
	...
	public function update( $new_settings, $old_settings ) {
		 ...
		 * @param Avangpress_Form_Widget $widget
		 * @ignore
		 */
		$new_settings = apply_filters( 'avangpress_form_widget_update_settings', $new_settings, $old_settings, $this );

		return $new_settings;
	}

} // class Avangpress_Widget
