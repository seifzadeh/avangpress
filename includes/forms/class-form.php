...
/**
 * Class Avangpress_Form
 *
 * Represents a Form object.
 *
 * To get a form instance, use `avangpress_get_form( $id );` where `$id` is the post ID.
 *
 * @access public
 * @since 3.0
 */
class Avangpress_Form {
    ...
    public static function get_instance( $post = 0 ) {
        ...
        if( $post instanceof WP_Post ) {
            ...
            if( empty( $post_id ) ) {
                $post_id = (int) get_option( 'avangpress_default_form_id', 0 );
            }
        }
        ...
        // check post object
        if( ! is_object( $post ) || ! isset( $post->post_type ) || $post->post_type !== 'avangpress-form' ) {
            $message = sprintf( __( 'There is no form with ID %d, perhaps it was deleted?', 'avangpress' ), $post_id );
            throw new Exception( $message );
        }

        $post_meta = get_post_meta( $post_id );
        $form = new Avangpress_Form( $post_id, $post, $post_meta );
    ...
    */
     ...
     *
     * @see Avangpress_Form_Element::get_response_html()
     *
     * @return string
     */
     ...
     * @param array $config
     * @return Avangpress_Form_element
     */
    public function get_element( $element_id = 'avangpress-form', $config = array() ) {
        return new Avangpress_Form_Element( $this, $element_id, $config );
    }

    /**
     * Get HTML string for this form.
     *
     * If you want to output a form, use `avangpress_show_form` instead as it.
     *
     * @param string $element_id
     * @param array $config
     *
     * @return string
     */
    public function get_html( $element_id = 'avangpress-form', array $config = array() ) {
    ...
    protected function load_settings( $post_meta = array() ) {
        ...
        if( ! $default_settings ) {
            $default_settings = include Avangpress_PLUGIN_DIR . 'config/default-form-settings.php';
        }
         ...
         */
        $settings = (array) apply_filters( 'avangpress_form_settings', $settings, $form );
    ...
    protected function load_messages( $post_meta = array() ) {
        ...
        $default_messages = include Avangpress_PLUGIN_DIR . 'config/default-form-messages.php';
         ...
         * @param Avangpress_Form $form
         */
        $messages = (array) apply_filters( 'avangpress_form_messages', $messages, $form );
    ...
    */
    public function add_notice( $text, $type = 'notice' ) {
        $this->notices[] = new Avangpress_Form_Notice( $text, $type );
    }
    ...
    public function __toString() {
        return avangpress_show_form( $this->ID, array(), false );
    }
    ...
    public function get_redirect_url() {
         ...
         * @param Avangpress_Form $form
         */
        $url = (string) apply_filters( 'avangpress_form_redirect_url', $url, $form );
        return $url;
    }
    ...
    public function validate() {
        ...
        if( empty( $errors ) ) {
            ...
            foreach( $this->get_required_fields() as $field ) {
                $value = avangpress_array_get( $this->data, $field );
         ...
         * @param Avangpress_Form $form
         */
        $errors = (array) apply_filters( 'avangpress_form_errors', $errors, $form );

        /**
         * @ignore
         * @deprecated 3.0 Use `avangpress_form_errors` instead
         */
        $form_validity = apply_filters( 'avangpress_valid_form_request', true, $this->data );
    ...
    protected function parse_request_data( array $data ) {
         ...
         * @param Avangpress_Form $form The form instance.
         */
        $ignored_field_names = apply_filters( 'avangpress_form_ignored_field_names', $ignored_field_names, $form );
    ...
    /**
     * Get Mail lists this form subscribes to
     *
     * @return array
     */
    public function get_lists() {
        ...
        /**
         * Filters Mail lists new subscribers should be added to.
         *
         * @param array $lists
         */
        ...
        /**
         * Filters Mail lists new subscribers coming from this form should be added to.
         *
         * @param array $lists
         * @param Avangpress_Form $form
         */
        $lists = (array) apply_filters( 'avangpress_form_lists', $lists, $form );
     ...
     *
     * @see `avangpress_form_errors` filter.
     * @return bool
     */
    ...
    public function get_data() {
         ...
         * @param Avangpress_Form $form
         */
        $data = apply_filters( 'avangpress_form_data', $data, $form );
    ...
    public function get_required_fields() {
         ...
         *
         * - All fields which are required for the selected Mail lists
         * - All fields in the form with a `required` attribute.
         *
         * @param array $required_fields
         * @param Avangpress_Form $form
         */
        $required_fields = (array) apply_filters( 'avangpress_form_required_fields', $required_fields, $form );

        return $required_fields;
    }

    /**
     * Get "email_type" setting for new Mail subscribers added by this form.
     *
     * @return string
     */
    ...
    */
    public function get_message( $key ) {
        ...
        if( $key === 'no_lists_selected' && current_user_can( 'manage_options' ) ) {
            $message .= sprintf( ' (<a href="%s">%s</a>)', avangpress_get_edit_form_url( $this->ID, 'settings' ), 'edit form settings' );
        }
